...
char game_get_cell(Game *g, int x, int y) { return g->cells[y * g->N + x]; }

void game_set_cell(Game *g, int x, int y, char c) {
  g->cells[y * g->N + x] = c;
}

Player *game_cur_player(Game *g) { return (g->cur == 1) ? &g->p1 : &g->p2; }

Player *game_other_player(Game *g) { return (g->cur == 1) ? &g->p2 : &g->p1; }

void game_switch_turn(Game *g) { g->cur = (g->cur == 1) ? 2 : 1; }

int game_board_full(Game *g) { return g->moves_count >= g->N * g->N; }

int game_check_winner(Game *g, char *out_color) {
  if (check_square_for_color(g, 'W')) {
    *out_color = 'W';
    return 1;
  }
  if (check_square_for_color(g, 'B')) {
    *out_color = 'B';
    return 1;
  }
  return 0;
...
static int check_square_for_color(Game *g, char C) {
  ...
  for (int ay = 0; ay < N; ay++) {
    for (int ax = 0; ax < N; ax++) {
      if (game_get_cell(g, ax, ay) != C)
        continue;
      for (int by = ay; by < N; by++) {
        for (int bx = (by == ay ? ax + 1 : 0); bx < N; bx++) {
          if (game_get_cell(g, bx, by) != C)
            continue;
          ...
          if (game_in_bounds(g, cx, cy) && game_in_bounds(g, dx2, dy2) &&
...
static int would_win(Game *g, int x, int y, char C) {
  if (!game_in_bounds(g, x, y))
    return 0;
  if (game_get_cell(g, x, y) != '.')
    return 0;
  game_set_cell(g, x, y, C);
  g->moves_count++;
...
static int find_winning(Game *g, char C, int *rx, int *ry) {
  for (int y = 0; y < g->N; y++)
    for (int x = 0; x < g->N; x++)
      if (game_get_cell(g, x, y) == '.' && would_win(g, x, y, C)) {
        *rx = x;
        *ry = y;
        return 1;
      }
...
void game_comp_move(Game *g, int *outx, int *outy) {
  ...
  int freecount = 0;
  for (int yy = 0; yy < g->N; yy++)
    for (int xx = 0; xx < g->N; xx++)
      if (game_get_cell(g, xx, yy) == '.')
        freecount++;
